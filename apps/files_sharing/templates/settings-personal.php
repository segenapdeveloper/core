...
<form class="section" id="files_sharing_settings">
	...
	<?php foreach ($_['enabled_configs'] as $key => $value): ?>
		<?php if ($value['enabled'] === 'yes'): ?>
			<input type="checkbox" name="<?php echo $key?>" id="<?php echo $key . '_input'?>" class="checkbox" value="1" checked="checked" />
		<?php else: ?>
			<input type="checkbox" name="<?php echo $key?>" id="<?php echo $key . '_input'?>" class="checkbox" value="1" />
		<?php endif; ?>
